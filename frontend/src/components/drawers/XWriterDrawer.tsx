...
const ENGAGEMENT_CONFIG = {
  ...
  low:    { label: 'Low Engagement',  color: 'text-slate-500',  bar: 'bg-slate-600', width: 'w-[28%]' },
};
...
const TweetPreview: React.FC<{ text: string; companyName: string }> = ({ text, companyName }) => {
  ...
  return (
    <div className="rounded-2xl border border-slate-700 bg-[#000] p-4">
      {/* Author */}
      <div className="flex items-start gap-3 mb-3">
        ...
        <div>
          ...
          <span className="text-[12px] text-slate-500">@{companyName.toLowerCase().replace(/\s+/g, '')}</span>
        </div>
      </div>

      {/* Tweet content */}
      <p className="text-[15px] text-white leading-[1.5] whitespace-pre-wrap mb-3">
        {text || <span className="text-slate-600">Your tweet will appear here…</span>}
      </p>

      {/* Meta */}
      <div className="text-[12px] text-slate-500 border-t border-slate-800 pt-2.5 mt-2.5">
        {timeStr} · <span className="text-white font-medium">X for AI CMO</span>
      </div>

      {/* Actions row */}
      <div className="flex items-center justify-around mt-3 pt-2.5 border-t border-slate-800 text-slate-600">
        {['💬', '🔄', '❤', '📊', '↗'].map((icon, i) => (
          <span key={i} className="text-[16px] hover:text-slate-400 cursor-pointer transition">{icon}</span>
        ))}
      </div>
    </div>
  );
...
const PostCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-slate-200 font-medium line-clamp-2 leading-snug mb-1.5">
        {post.hook}
      </p>
      <div className="flex items-center gap-2">
        ...
        <span className={`text-[10px] ${post.charCount > 280 ? 'text-red-400' : 'text-slate-600'}`}>
          {post.charCount}/280
        </span>
      </div>
    </motion.button>
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            {/* ── Header ── */}
            <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-slate-800 bg-black/40">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-black border border-slate-700 flex items-center justify-center text-white">
                  <XLogo />
                </div>
                <div>
                  <h2 className="text-sm font-bold text-white">X Writer</h2>
                  <p className="text-[11px] text-slate-500">AI-native tweet generator · founder tone</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {feed && (
                  <span className="text-[10px] px-2 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-400">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                  {feed ? (
                    ...
                    <div className="py-6 flex flex-col items-center gap-2">
                      {[0,1,2].map(i => (
                        <div key={i} className="w-full h-14 rounded-xl bg-slate-800/50 animate-pulse" />
                      ))}
                    </div>
                  ) : (
                    <div className="py-8 text-center px-2">
                      <p className="text-[11px] text-slate-600">Generate posts to see them here</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              {selected ? (
                <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
                  ...
                  <div className="shrink-0 px-4 py-2.5 border-b border-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-slate-600">{ANGLE_META[selected.angle].desc}</span>
                    </div>
                    ...
                    {tab === 'preview' ? (
                      <>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1.5">
                            <p className="text-[10px] uppercase tracking-wider text-slate-500">Edit</p>
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-[11px] font-mono ${overLimit ? 'text-red-400' : 'text-slate-500'}`}>
                                {280 - charCount}
                              </span>
                            </div>
                          </div>
                          <textarea
                            ...
                            className="w-full bg-slate-900/40 border border-slate-700 focus:border-slate-500 rounded-xl text-sm text-slate-200 leading-relaxed resize-none outline-none px-4 py-3 min-h-[100px] transition-colors"
                          />
                        </div>
                      </>
                    ) : (
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="rounded-xl border border-slate-800 bg-slate-900/30 p-4 space-y-3">
                          <p className="text-[10px] uppercase tracking-wider text-slate-500">Post Structure</p>
                          <div className="space-y-2">
                            <div>
                              <p className="text-[10px] text-slate-500 mb-1">Hook</p>
                              <p className="text-xs text-slate-200 font-medium">{selected.hook}</p>
                            </div>
                            <div className="h-px bg-slate-800" />
                            <div>
                              <p className="text-[10px] text-slate-500 mb-1">Body</p>
                              <p className="text-xs text-slate-400 whitespace-pre-wrap">{selected.body}</p>
                            </div>
                            <div className="h-px bg-slate-800" />
                            <div>
                              <p className="text-[10px] text-slate-500 mb-1">CTA</p>
                              <p className="text-xs text-slate-300">{selected.cta}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className="flex-1 flex items-center justify-center">
                  <div className="text-center px-6">
                    <div className="w-12 h-12 rounded-xl bg-black border border-slate-800 flex items-center justify-center text-slate-600 mx-auto mb-3">
                      <XLogo />
                    </div>
